...
description: 用于对由 skill-dev-link 接入的 Codex skill 仓库执行 Git 状态查看、远程拉取、推送等同步操作。当用户希望同步远程 skill 仓库、把本地 skill 改动推送到远程，或让其他 skill 统一转调仓库同步能力时使用。
---
...
它读取 `~/.codex/skills/.dev-links.json`，通过 skill 名称定位对应的 Git 仓库，然后对该仓库执行 `status`、`pull`、`push`。

注意：命令参数中的 `--name <skill-name>` 只是用来定位仓库；一旦执行 `pull` 或 `push`，影响的是这个 skill 所在的整个仓库，而不是单个目录。
...
- `skill-update` 可以把仓库同步类请求转调到本 skill 的脚本，但普通 `update` 仍然只负责安装更新
