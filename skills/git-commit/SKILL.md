---
name: git-commit
description: 用于在本地仓库完成 git 提交流程：分析未暂存改动、询问排除文件、按功能点拆分提交、根据仓库现有规范生成提交信息并执行提交。用户提到 git 提交、提交信息、提交规范、拆分提交或提交失败需要重试等场景时使用。
---
...
3. 按功能点拆分提交
   - 根据修改目的分组，优先使用 `git add -p` 或逐文件暂存。
...
4. 发现仓库提交规范
   ...
   - 若存在模板或规范，严格遵循。
   - 若找不到规范，询问用户提供格式要求，不要自造规则。

5. 生成提交信息
   - 根据规范生成标题/正文/脚注，按仓库语言要求编写。
